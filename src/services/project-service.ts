import { ProjectCategory, ProjectList, ProjectDetail, ProjectDetailDto } from "../interfaces/ProjectInterface";
...
export const createProjectAuthorize = async (data: ProjectDetailDto) => {
    ...
    return { message: response.data.message as string, data: response.data.content as ProjectDetailDto };
}

//updateProject
export const updateProject = async (data: ProjectDetailDto) => {
    const response = await apiClient.put(`/Project/updateProject?projectId=${data.id}`, {
        "id": data.id,
        "projectName": data.projectName,
    ...
    return { message: response.data.message as string, data: response.data.content };
}

//deleteProject
export const deleteProject = async (id: number) => {
    const response = await apiClient.delete(`/Project/deleteProject?projectId=${id}`);
    return { message: response.data.message as string, data: response.data.content[0] as number };
}